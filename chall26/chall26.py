...
def encryptUserInput(userInput, password, nonce):
	...
	while i<len(userInput):
		if userInput[i] ==';' or userInput[i]=='=':
			...
			i+=2
		i+=1
	userInput=bytearray(prefix+userInput+postfix, "UTF-8")
	return CTR(userInput, expandedkey, nonce)

def decryptUserInput(encrypted, password, nonce):
	expandedkey=expandKey(password)
	decrypted=CTR(encrypted, expandedkey, nonce)
...
encrypted=encryptUserInput(":admin+true", password, nonce)
encrypted[32]=encrypted[32]^ord(":")^ord(";")
encrypted[38]=encrypted[38]^ord("+")^ord("=")
decryptUserInput(encrypted, password, nonce)