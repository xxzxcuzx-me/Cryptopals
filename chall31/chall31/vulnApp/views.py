...
def verifyHMAC(message, signature, key):
    hmac=hmacSHA1(message, key)
    hmac=''.join(format(x, '02x') for x in hmac)
    for i in range(40):
...
def index(request):
    ...
    file = bytearray(request.GET.get('file'), "UTF-8")
    signature = request.GET.get('signature')
    if verifyHMAC(file, signature, key):
